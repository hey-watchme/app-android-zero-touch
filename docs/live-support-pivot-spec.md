---
作成日: 2026-04-28
更新: 2026-04-29（v4: Live基盤実装・本番反映状況を追記）
ステータス: ドラフト（実装進行中）
...
## 0.1 実装の最新状態（2026-04-29）

仕様だけでなく、現在の実装・デプロイ状態をここに固定する。

- backend 最終デプロイ: `2d8cc98`（Run `25064282756` 成功）
- Android 最新実装: `f03aca9`（Live transcript UX + ローカル保持 + サイドバー状態保持まで反映）
- 本番APIヘルス: `https://api.hey-watch.me/zerotouch/health` -> `200 OK`
- 本番 OpenAPI に Live 追加APIが存在:
  - `/api/live-sessions`
  - `/api/transcribe/realtime`
- Supabase に Live 関連テーブル適用済み:
  - `zerotouch_live_sessions`
  - `zerotouch_live_transcripts`
  - `zerotouch_live_keypoints`

再開時の運用手順・確認コマンドは `docs/live-support-next-session-handoff.md` を正本とする。
