...
- Live Support 機能の現状仕様（完成済み）: `docs/live-support.md`
- Conversation パイプラインの正本（再稼働対象）: `docs/conversation-visualization-pipeline.md`
- 将来 TODO（Wiki / Action Candidate / Connector など）: `docs/roadmap.md`
- 上位設計（最終形態の正本）: `docs/conversation-action-platform.md`

モード整理:

- `Live モード`: 会話中のリアルタイム表示 + Translation を提供する presentation surface（実装済み）
- `ZeroTouch モード`: Card / Topic / Fact / Wiki / Action Candidate を生成する本来の会話処理パイプライン（再稼働が必要）
...
- **会話可視化パイプライン正本（再稼働対象）**: `docs/conversation-visualization-pipeline.md`
- **Live Support 機能の現状仕様（完成済み）**: `docs/live-support.md`
- **将来の TODO（後段機能の構想）**: `docs/roadmap.md`
...
会話可視化については、今後は `docs/conversation-visualization-pipeline.md` を正本とする。
特に Home の `Conversation` は、
...
の順で進む本体 pipeline を表示するものとして扱う。
このとき正本として扱う対象は **Home 右側 `Conversation` レーンのみ** で、左側 `Live Support` や `Dashboard` は別扱いとする。

## 現在できていること

### ✅ Live Support path（動作中）

- `presentation path`: アンビエント録音 → realtime transcribe (`/api/transcribe/realtime`) → realtime translate (`/api/translate/realtime`) → Home の `Live Transcript / English Translation`
- 詳細: `docs/live-support.md`

### 🔴 Conversation path（**現在動いていない / 再稼働が必要**）

本来の設計:

- アンビエント録音 → S3アップロード → ASR文字起こし（Speechmatics / Deepgram / Azure Speech Service）→ Card生成 → active Topic へ即時紐付け → アプリ表示 → idle / ambient stop で Topic finalize → Topic / Fact / Wiki / Query
...
**直近の最優先タスクは、この Conversation path を再度動かすこと。**  
これが動くまで Wiki / Action Candidate / Connector などの後段機能には進まない。

参考メモ:

- 設計正本: `docs/conversation-visualization-pipeline.md`
- LLM カード生成は一旦停止中（`/api/generate-cards` は未使用）
- ここでいう `Card` は会話の最小セッション単位として `zerotouch_sessions` に生成されるもので、Live Transcript の表示行そのものではない
- コードベースには旧 `pending -> batch grouping` モデルや stateful 実験の痕跡が一部残っている

### 補助情報

**ASRプロバイダー**: Speechmatics / Deepgram / Azure Speech Service（バックエンドは稼働可能な状態。Conversation path 復活時に利用）  
**保留中**: Cohere（現在の ambient 録音形式 `m4a` をそのまま受け付けないため）

**Web 閲覧ダッシュボード**: `https://app-web-zero-touch.vercel.app/`  
Android 側の Topic / Card を Web で閲覧できる読み取り専用 MVP（ただし Conversation path 停止中はデータが流入しない）。

**Wiki Query API（バックエンド側は稼働中だが、上流の Conversation path が止まっているため新しい Wiki は生成されない）**
- Query API: `POST https://api.hey-watch.me/zerotouch/api/query-wiki`
- ログ取得 API: `GET https://api.hey-watch.me/zerotouch/api/wiki-log`
- 疎通確認:
...
## Action / Connector レイヤー（中長期ロードマップ）

※ これは **ZeroTouch モード側** の中長期ロードマップです。  
直近の最優先タスクは「**従来の Conversation パイプラインを再度動かすこと**」で、それが安定してからこの層に着手します。詳細は `docs/roadmap.md` を参照。
...
将来 TODO（Action Candidate / Connector / Take-Home / 多言語化など）はすべて `docs/roadmap.md` に集約しています。Conversation パイプライン復活後に着手します。

## 参考

- ドキュメント入口: `docs/README.md`
- Live Support 機能の現状仕様: `docs/live-support.md`
- 会話可視化パイプライン正本: `docs/conversation-visualization-pipeline.md`
- 将来 TODO: `docs/roadmap.md`
